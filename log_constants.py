#!/usr/bin/env python
# coding: utf-8

# In[1]:


#*******************************************************************************************
 #
 #  File Name:  log_constants.py
 #
 #  File Description:
 #      This Python script, log_constants.py, contains generic Python constants
 #      for documenting runtime information for review.
 #
 #
 #  Date            Description                             Programmer
 #  ----------      ------------------------------------    ------------------
 #  02/19/2024      Initial Development                     N James George
 #
 #******************************************************************************************/


# In[2]:


CONSTANT_LOCAL_FILE_NAME = 'log_constants.py'


# In[3]:


LOG_FLAG = False

IMAGE_FLAG = False

PROGRAM_DESIGNATION = ''

LOGS_DIRECTORY_PATH = '/content/drive/My Drive/alphabet_soup_charity_analysis/logs'

IMAGES_DIRECTORY_PATH = '/content/drive/My Drive/alphabet_soup_charity_analysis/images'

BASE_LOG_FILE_NAME = '_log.txt'

LOG_FILE_PATH = ''

log_txt_file = None


# In[ ]:




